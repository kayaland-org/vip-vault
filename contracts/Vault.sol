...
contract Vault is BasicVault {
    ...
    modifier notAllowContract() {
        require(!address(msg.sender).isContract(), "Vault.notContract: sender is contract ");
        _;
    }
    /// @notice not in lup
    modifier notInLup() {
        bool inLup = block.timestamp <= lup();
        require(!inLup, "BasicVault.notInLup: in lup");
        _;
    }
    ...
    function joinPool(uint256 amount) external isAllowJoin notAllowContract {
        ...
        require(amount <= ioToken().balanceOf(investor) && amount > 0, "Vault.joinPool: Insufficient balance");
    ...
    function exitPool(uint256 amount) external isAllowExit notInLup notAllowContract {
        address investor = msg.sender;
        require(balanceOf(investor) >= amount && amount > 0, "Vault.exitPool: Insufficient balance");
        //take exit fee
    ...
    function exitPoolOfUnderlying(uint256 amount) external isAllowExit notInLup notAllowContract {
        address investor = msg.sender;
        require(balanceOf(investor) >= amount && amount > 0, "Vault.exitPoolOfUnderlying: Insufficient balance");
        //take exit fee
        uint256 exitFee = _takeExitFee(investor, amount);
        uint256 exitAmount = amount.sub(exitFee);
        //take performance fee
