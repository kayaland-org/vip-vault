...
abstract contract BasicVault is ERC20, GovIdentity {
    ...
    modifier withinCap() {
        ...
        require(check, "BasicVault.withinCap: Cap limit reached");
    }

    /// @notice Prohibition of vault circulation
    modifier deny() {
        require(!SmartPoolStorage.load().suspend, "BasicVault.isNotSuspend: vault is suspend");
        _;
    }

    /// @notice is allow join
    modifier isAllowJoin() {
        require(checkAllowJoin(), "BasicVault.checkAllowJoin: vault is not allowJoin");
        _;
    }

    /// @notice is allow exit
    modifier isAllowExit() {
        require(checkAllowExit(), "BasicVault.checkAllowExit: vault is not allowExit");
        _;
    }
    ...
    function setFee(SmartPoolStorage.FeeType ft, uint256 ratio, uint256 denominator, uint256 minLine) external onlyAdminOrGovernance {
        require(ratio <= denominator, "BasicVault.setFee: ratio<=denominator");
        SmartPoolStorage.Fee storage fee = SmartPoolStorage.load().fees[ft];
        require(fee.denominator == 0, "BasicVault.setFee: already initialized ");
